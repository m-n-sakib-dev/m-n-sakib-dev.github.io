          ...
          {experiences.map((exp, i) => (
            ...
            >
              <div className="relative z-10 flex-shrink-0 md:absolute md:left-1/2 md:-translate-x-1/2 mt-1 md:mt-5">
                <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm font-bold ${exp.type === 'current'
              ...
              <div className={`md:w-1/2 pl-14 md:pl-0 ${i % 2 === 0
                ...
                <div className={`rounded-2xl p-5 border border-slate-200 dark:border-white/10 bg-white glass shadow-md dark:shadow-none card-hover ${exp.type === 'current' ? 'glow-primary' : ''
                  }`}>
                  {exp.type === 'current' && (
                  ...
                  <p className="text-base font-semibold text-purple-600 dark:text-purple-400 mb-0.5">{exp.company}</p>
                  <p className="text-sm text-slate-500 dark:text-slate-400 font-mono mb-4">{exp.period}</p>
                  <ul className={`space-y-2 ${i % 2 === 0 ? 'md:text-left' : ''}`}>
                    {exp.bullets.map((b, bi) => (
                      <li key={bi} className="flex items-start gap-2 text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
